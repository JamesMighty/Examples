...
class Resolver:
    ...
    def Resolve(self, three=None, inp=None, allResolvedMatches=[], lastMatch=0, doPrintUnresolved=False, syntaxSetting=SyntaxE.Ahead, doOnlyOne=False):
        ...
        todo = []
        if inp == None:
            inp = strip_accents(input(f"{self.Context['username']:{self.Context['padding']}}{self.Context['indexation']} ").lower())
        else:
        ...
        for comm in three: 
            ...
            if len(matches) > 0:
                ...
                if not any_common([match.String for match in matches],[resolvedMatch.String for resolvedMatch in allResolvedMatches]) and self.SyntaxCheck(lastMatch, matchOn, syntaxSetting):
