...
def node_has_specified_syntax_flag(node,flagName, arg):
    ...
    if SyntaxE.Ahead in flag or SyntaxE.Slack in flag:
        assert len(todo[0][2]) > 0, "node does not have any sub-todolist when node before args"
    if SyntaxE.Next in flag:
        assert len(todo[0][2]) == 0, "node does have sub-todolist when node is after args"
    ...
    if SyntaxE.Next in flag or SyntaxE.Slack in flag:
        assert len(todo[0][2]) > 0, "node does not have sub-todolist when node is after args"
